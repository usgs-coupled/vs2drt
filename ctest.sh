...
do
	...
	module load cmake/${CMAKE_VER}
	module load intel/psxe-${INTEL_VER}
	ctest -S intel.cmake -C Release -VV -O cmake-${CMAKE_VER}-intel_psxe-${INTEL_VER}.log -DBUILD_MPI:BOOL=OFF -DBUILD_OPENMP:BOOL=ON -DCMAKE_VER:STRING=${CMAKE_VER} -DINTEL_VER:STRING=${INTEL_VER}
	
        # mpi
	module purge
	module load cmake/${CMAKE_VER}
	module load intel/psxe-${INTEL_VER}
	ctest -S intel.cmake -C Release -VV -O cmake-${CMAKE_VER}-intel_psxe-${INTEL_VER}.log -DBUILD_MPI:BOOL=ON -DBUILD_OPENMP:BOOL=OFF -DCMAKE_VER:STRING=${CMAKE_VER} -DINTEL_VER:STRING=${INTEL_VER}
    done
done
